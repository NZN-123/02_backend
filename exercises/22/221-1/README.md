...
총 6개의 단계별 실습 예제로 구성되어 있으며, 순차적으로 진행합니다.
...
- 터미널에서 컴파일 및 실행:
  ...
  java Ex02_MethodSignature
  java Ex03_ParameterPassing
  java Ex04_ConstructorAndThis
  java Ex05_StaticConstraint
  java Ex06_InnerClassMemoryLeak
  ```
...
### 2단계: 메서드 선언과 시그니처 (`Ex02_MethodSignature.java`)
- **목표**: 메서드의 매개변수, 반환 타입, `return`, 가변인자와 메서드 시그니처의 구성 요소 이해
- **실습 내용**:
  - 매개변수(parameter)와 인자(argument)의 차이 및 반환값 사용
  - 메서드 이름과 매개변수 타입·순서로 결정되는 메서드 시그니처 확인
  - 반환 타입만 다른 메서드는 오버로딩할 수 없는 이유 분석
  - 가변인자(`type... name`) 선언 규칙과 배열로 처리되는 호출 방식 확인
...
### 4단계: 생성자 오버로딩 & `this` 체이닝 (`Ex04_ConstructorAndThis.java`)
...
### 5단계: Static 제약 조건 (`Ex05_StaticConstraint.java`)
...
### 6단계: 내부 클래스와 메모리 누수 (`Ex06_InnerClassMemoryLeak.java`)
